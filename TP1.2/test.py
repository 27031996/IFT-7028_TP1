...
l = 12*60  # temps moyen d'arrivé des bateaux en minute
s = 9.5  # temps moyen de charg/décharg pour 2 robots
t_simul=40000*60 # temps d'horizon pour la simulation
r = 2  # nombre des robots

# generating inter arrival times using exponential distribution

inter_arrival_times = list(np.random.exponential(scale=l*60))
...
data = pd.DataFrame(list(zip(arrival_times, service_times, total_times, finish_times, wait_times, inter_arrival_times)),
                    columns=['arrival_times', 'service_times', 'total_times', 'finish_times', 'wait_times',
                             'inter_arrival_times'])
...
print('Output:', '\n',
      ...
      'Service Time: (1/µ)', str(data.service_times.mean()), '\n'
                                                             ' Utilization (c): ',
      str(workingtime / timeline.time.max()), '\n',
